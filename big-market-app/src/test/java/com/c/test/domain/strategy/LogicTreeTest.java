...
public class LogicTreeTest {
    ...
    public void test_tree_rule() {
        // 1. 构建 [规则锁] 节点及连线
        RuleTreeNodeVO rule_lock = RuleTreeNodeVO.builder().treeId(String.valueOf(100000001)).ruleKey("rule_lock")
                                                 .ruleDesc("抽奖次数锁判定")
                                                 .ruleValue("1")
                                                 .treeNodeLineVOList(new ArrayList<RuleTreeNodeLineVO>() {{
                                                     // 连线1：锁拦截 -> 转幸运奖
                                                     add(RuleTreeNodeLineVO.builder().treeId(String.valueOf(100000001))
        ...
        RuleTreeNodeVO rule_luck_award = RuleTreeNodeVO.builder().treeId(String.valueOf(100000001))
                                                       .ruleKey("rule_luck_award")
                                                       .ruleDesc("兜底幸运奖励").ruleValue("1").treeNodeLineVOList(null)
                                                       .build();

        // 3. 构建 [库存规则] 节点及连线
        RuleTreeNodeVO rule_stock = RuleTreeNodeVO.builder().treeId(String.valueOf(100000001)).ruleKey("rule_stock")
                                                  .ruleDesc("库存扣减校验")
                                                  .ruleValue(null)
                                                  .treeNodeLineVOList(new ArrayList<RuleTreeNodeLineVO>() {{
                                                      // 连线1：库存不足 -> 转幸运奖
        ...
        DefaultTreeFactory.StrategyAwardVO data = treeEngine.process("user_001", 100001L, 100, null);
